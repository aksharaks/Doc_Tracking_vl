...
{
    [SessionTimeout]
    public class Req_BH_VLController : Controller
    {
        ...
        // GET: Req_BH_VL
        public ActionResult Req_VL_BH()
        {
            Req_VL_BH model = new Req_VL_BH();
            ...
            if (Session["user_id"] != null)
            {
                ...
                con.Open();
                string sql = "select distinct d.loan_no from documents_tracking_vl d, employee_master e, branch_master s where d.branch_id = s.branch_id and s.branch_id = e.branch_id and d.return_status=1 and e.emp_code='" + id + "'";
        ...
        [HttpPost]
        public ActionResult Search_doc(Req_VL_BH v, string loanMode)
        {
            try
            {
                MaafinDbHelper mdh = new MaafinDbHelper();
                ...
                if (Session["user_id"] != null)
                {
                    ...
                    if (v.SelectedDocType == "Loan Document" && loanMode == "vehicle")
                    {
                        ...
                        v.LoanDocsReq_vl_bh = new LoanDocRequ_Mortgage_BH_VL
                        {
                            LoanDocs = new List<LoanDoc_BH_vl>()
                        };
                        ...
                        while (loanDocReader.Read())
                        {
                            ...
                            v.LoanDocsReq_vl_bh.LoanDocs.Add(loanDoc);
                        }
                        ...
                        return PartialView("_LoanDoc_VL_BH_Req", v);
                    }
        ...
        [HttpPost] 
        public ActionResult Save_LoanReq_Mortgage_VL_BH(List<Req_LoanDoc_Mort_vl_bh> model, string selectedLoanNumber, string selectedDocType)
        {
            MaafinDbHelper mdh = new MaafinDbHelper();
            mdh.Connection();
            constr = mdh.conStr1;
            ...
            try
            {
                using (OracleConnection con = new OracleConnection(constr))
                {
                    ...
                    using (OracleCommand cmdCheckTracking = new OracleCommand(sqlCheckDocumentsBranchTracking, con))
                    {
                        ...
                        {
                            // Update DOCUMENTS_BRANCH_TRACKING
                            string sqlUpdateDocumentsBranchTracking1 = "UPDATE documents_tracking_vl t SET t.return_status=2 WHERE t.loan_no = :loan_number AND t.type_of_docs = :doc_type and t.return_status=1 ";
