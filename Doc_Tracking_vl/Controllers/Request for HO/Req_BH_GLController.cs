...
namespace Doc_Tracking_vl.Controllers.Request_for_HO
{
    ...
    [SessionTimeout]
    public class Req_BH_GLController : Controller
    {
        // GET: Req_BH_GL
        ...
        OracleDataReader dr;
        public ActionResult Req_GL_BH()
        {
            Req_GL_BH model = new Req_GL_BH();
            ...
            if (Session["user_id"] != null)
            {
                ...
                con.Open();
                string sql = "select distinct d.loan_no from documents_tracking_vl d, employee_master e, branch_master s where d.branch_id = s.branch_id and s.branch_id = e.branch_id and d.return_status=3 and e.emp_code='" + id + "'";
        ...
        [HttpPost]
        public ActionResult Search_doc(Req_GL_BH v, string loanMode)
        {
            try
            {
                MaafinDbHelper mdh = new MaafinDbHelper();
                ...
                if (Session["user_id"] != null)
                {
                    ...
                    if (v.SelectedDocType == "Loan Document" && loanMode == "vehicle")
                    {
                        ...
                        v.LoanDocsReq_gl_bh = new LoanDocRequ_Mortgage_BH_GL
                        {
                            LoanDocs = new List<LoanDoc_BH_gl>()
                        };
                        ...
                        while (loanDocReader.Read())
                        {
                            ...
                            v.LoanDocsReq_gl_bh.LoanDocs.Add(loanDoc);
                        }
                        ...
                        return PartialView("_LoanDoc_GL_BH", v);
                    }
        ...
        [HttpPost]
        public ActionResult Save_LoanReq_Mortgage_GL_BH(List<Req_LoanDoc_Mort_gl_bh> model, string selectedLoanNumber, string selectedDocType)
        {
            MaafinDbHelper mdh = new MaafinDbHelper();
            mdh.Connection();
            constr = mdh.conStr1;
            ...
            try
            {
                using (OracleConnection con = new OracleConnection(constr))
                {
                    ...
                    using (OracleCommand cmdCheckTracking = new OracleCommand(sqlCheckDocumentsBranchTracking, con))
                    {
                        ...
                        {
                            // Update DOCUMENTS_BRANCH_TRACKING
                            string sqlUpdateDocumentsBranchTracking1 = "UPDATE documents_tracking_vl t SET t.return_status=4 WHERE t.loan_no = :loan_number AND t.type_of_docs = :doc_type and t.return_status=3";
